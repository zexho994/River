...
public class CollectionStreamGenerator {
     ...
     * 将一个List转化为stream流
     * */
    public static <T> PipStream<T> getListStream(List<T> list){
     ...
     * */
    private static <T> PipStream<T> getListStream(Iterator<T> iterator, boolean isStart){
        if(!iterator.hasNext()){
            // 不存在迭代的下一个元素，返回空的流
        ...
        if(isStart){
            // 初始化，只需要设置 求值过程
            return new PipStream.Builder<T>()
